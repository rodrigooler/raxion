...
type LocaleOverrides<T> = Partial<Record<Exclude<Locale, "en">, T>>;
type Localized<T> = Record<Locale, T>;
type SharedSectionSpec = [heading: string, paragraphs: string[], bullets?: string[]];
type BlogSpec = [
  slug: string,
  date: string,
  title: string,
...
function localize<T>(en: T, overrides: LocaleOverrides<T> = {}): Localized<T> {
  return {
    en,
    "pt-BR": overrides["pt-BR"] ?? en,
    zh: overrides.zh ?? en,
    ja: overrides.ja ?? en,
    es: overrides.es ?? en,
  };
...
const text = (en: string, overrides: LocaleOverrides<string> = {}) => localize(en, overrides);
const lines = (en: string[], overrides: LocaleOverrides<string[]> = {}) => localize(en, overrides);

function section(input: RichSection): RichSection {
  return input;
}

function sharedSection([heading, paragraphs, bullets]: SharedSectionSpec): RichSection {
  return section({
    heading: text(heading),
    paragraphs: lines(paragraphs),
    bullets: bullets ? lines(bullets) : undefined,
  });
}

function createPost(input: Omit<Post, "author">): Post {
  return { ...input, author };
}

function englishBlog([slug, date, title, summary, sections]: BlogSpec): Post {
  return createPost({
    type: "blog",
    slug,
    date,
    ...
    body: sections.map(sharedSection),
  });
}
...
const englishBlogSpecs: BlogSpec[] = [
  [
    "coherence-score-is-now-running-code",
    "2026-02-23",
  ...
  [
    "q1-complete-heres-what-we-proved",
    "2026-02-25",
    "Q1 Complete. Here's What We Proved — and What We Didn't.",
  ...
  [
    "how-to-build-a-smart-agent-on-raxion",
    "2026-02-26",
...
function announcementRebuild(): Post {
  return createPost({
    type: "announcement",
    slug: "vinext-site-rebuild",
    date: "2026-02-20",
    title: text("Site rebuild completed on vinext", {
    ...
    summary: text(
      "The public site now runs as a component-based vinext app with a local Tailwind pipeline and a shared visual system.",
      {
    ...
    body: [
      section({
        ...
        bullets: lines(
          ["Componentized sections", "Shared announcement banner", "Localized route support"],
          {
      ...
      section({
        ...
        paragraphs: lines(
          ...
          ],
          {
            ...
            zh: ["这让迭代更快：公告、长文、技术更新和未来产品页面都可以复用同一套视觉原语，而不必复制标记。"],
            ja: ["これにより、告知、長文、技術更新、今後の製品ページが同じ視覚プリミティブを再利用でき、マークアップの重複を避けられます。"],
            es: ["Esto permite iteracion mas rapida: anuncios, ensayos, actualizaciones tecnicas y futuras paginas de producto pueden reutilizar los mismos bloques visuales sin duplicar markup."],
          }
        ),
      }),
    ],
...
function whitepaperLaunchPost(): Post {
  return createPost({
    type: "blog",
    slug: "whitepaper-v1-live-building-in-public",
    date: "2026-02-22",
    title: text("RAXION Whitepaper v0.4 Is Live - And We're Building in Public", {
    ...
    body: [
      section({
        heading: text("What We Published", {
        ...
        bullets: lines(
          ...
          {
            "pt-BR": ["Capitulo 1 - A crise da subjetividade", "Capitulo 1.5 - O que nao somos", "Capitulo 2 - Arquitetura da Neural SVM", "Capitulo 3 - PoIQ: o protocolo da verdade", "Capitulos 4 e 5 - Tokenomics + roadmap"],
            zh: ["第 1 章 - 主观性危机", "第 1.5 章 - 我们不是什么", "第 2 章 - Neural SVM 架构", "第 3 章 - PoIQ：真相协议", "第 4 和第 5 章 - Tokenomics + 路线图"],
            ja: ["第 1 章 - 主観性の危機", "第 1.5 章 - 私たちが目指さないもの", "第 2 章 - Neural SVM アーキテクチャ", "第 3 章 - PoIQ: 真理のプロトコル", "第 4 章と第 5 章 - Tokenomics + ロードマップ"],
            es: ["Capitulo 1 - La crisis de la subjetividad", "Capitulo 1.5 - Lo que no somos", "Capitulo 2 - Arquitectura Neural SVM", "Capitulo 3 - PoIQ: el protocolo de la verdad", "Capitulos 4 y 5 - Tokenomics + hoja de ruta"],
          }
        ),
      }),
      section({
        heading: text("What We're Honest About", {
        ...
        bullets: lines(
          ...
          {
            "pt-BR": ["Arquiteturas heterogeneas convergem em mais de 70% das consultas do mundo real", "PoIQ permanece seguro sob incentivos economicos reais e atores adversarios", "O roadmap de latencia e viavel", "Desenvolvedores consideram a proposta forte o bastante para construir agentes"],
            zh: ["异构架构在超过 70% 的真实查询中能够收敛", "PoIQ 在真实经济激励和对抗性参与者下仍然安全", "延迟路线图具备可达成性", "开发者认为这一价值主张足够强，愿意基于它构建代理"],
            ja: ["異種アーキテクチャは現実のクエリの 70% 超で収束する", "PoIQ は実際の経済インセンティブと敵対的参加者の下でも安全性を維持する", "レイテンシのロードマップは到達可能である", "開発者がエージェントを構築するだけの価値提案がある"],
            es: ["Las arquitecturas heterogeneas convergen en mas del 70% de las consultas del mundo real", "PoIQ sigue siendo seguro bajo incentivos economicos reales y actores adversarios", "La hoja de ruta de latencia es alcanzable", "Los desarrolladores consideran que la propuesta de valor es suficientemente fuerte para construir agentes"],
          }
        ),
      }),
      section({
        heading: text("What Happens Next", {
        ...
        paragraphs: lines(
          ...
          {
            "pt-BR": ["Estamos na Fase 0 - Genese. O Q1 de 2026 inclui o Whitepaper v0.4, o PoC em Python, um esqueleto de proof-of-execution com RISC Zero e um repositorio com issues prontas para contribuidores.", "O Q2 de 2026 e Devnet: o fork de Agave, o PoIQ v0.1 on-chain via Anchor, o Cognitive Scheduler e o Agent SDK em Rust."],
            zh: ["我们目前处于阶段 0 - 创世。2026 年第一季度包括 Whitepaper v0.4、Python PoC、RISC Zero proof-of-execution 脚手架，以及一个已经整理好贡献入口的仓库。", "2026 年第二季度是 Devnet：Agave 分叉、通过 Anchor 上链的 PoIQ v0.1、Cognitive Scheduler，以及 Rust 版 Agent SDK。"],
            ja: ["現在はフェーズ 0 - Genesis です。2026 年第 1 四半期には Whitepaper v0.4、Python PoC、RISC Zero の proof-of-execution スキャフォールド、そしてコントリビュータ向け課題を整理したリポジトリが含まれます。", "2026 年第 2 四半期は Devnet です。Agave フォーク、Anchor 経由でオンチェーン化する PoIQ v0.1、Cognitive Scheduler、そして Rust の Agent SDK が入ります。"],
            es: ["Estamos en Fase 0 - Genesis. El Q1 de 2026 incluye el Whitepaper v0.4, el PoC en Python, una base de proof-of-execution con RISC Zero y un repositorio con issues listos para contribuidores.", "El Q2 de 2026 es Devnet: el fork de Agave, PoIQ v0.1 on-chain via Anchor, el Cognitive Scheduler y el Agent SDK en Rust."],
          }
        ),
      }),
      section({
        heading: text("Read It", {
          "pt-BR": "Leia agora",
          zh: "立即阅读",
          ja: "読む",
          es: "Leer ahora",
        ...
        paragraphs: lines(
          ...
          {
            "pt-BR": ["O whitepaper completo esta publicado em github.com/rodrigooler/raxion/blob/main/WHITEPAPER.md.", "Se voce encontrar uma falha na matematica, abra uma issue. Se discordar de uma decisao de design, abra uma issue. O documento existe para ser contestado."],
            zh: ["完整白皮书已发布在 github.com/rodrigooler/raxion/blob/main/WHITEPAPER.md。", "如果你发现数学上的漏洞，请开 issue；如果你不同意某个设计决策，也请开 issue。这个文档本来就应该接受质疑。"],
            ja: ["完全版ホワイトペーパーは github.com/rodrigooler/raxion/blob/main/WHITEPAPER.md で公開中です。", "数式に欠陥を見つけたら issue を開いてください。設計判断に異論があっても issue を開いてください。この文書は検証と反論を前提にしています。"],
            es: ["El whitepaper completo ya esta publicado en github.com/rodrigooler/raxion/blob/main/WHITEPAPER.md.", "Si encuentras una falla en la matematica, abre un issue. Si no estas de acuerdo con una decision de diseno, abre un issue. Este documento existe para ser discutido."],
          }
        ),
      }),
    ],
...
function whyRaxionExistsPost(): Post {
  return createPost({
    type: "blog",
    slug: "why-raxion-exists",
    date: "2026-02-20",
    title: text("Why RAXION exists", {
    ...
    body: [
      section({
        ...
        paragraphs: lines(
          ...
          {
            "pt-BR": ["A maioria dos sistemas de IA descentralizada ainda depende de humanos para decidir qual modelo e bom. Isso significa que o gargalo de confianca nunca desapareceu; ele so foi tokenizado.", "A RAXION existe para substituir essa camada subjetiva por convergencia objetiva e verificavel entre sistemas de inferencia complementares."],
            zh: ["大多数去中心化 AI 系统仍然依赖人类来判断哪个模型更好。这意味着信任瓶颈并没有消失，只是被代币化了。", "RAXION 的目标是用互补推理系统之间可验证的客观收敛，取代这层主观判断。"],
            ja: ["多くの分散型 AI システムは、どのモデルが優れているかを人間に判断させています。つまり信頼のボトルネックは消えておらず、トークン化されただけです。", "RAXION は、その主観レイヤーを、補完的な推論システム間の客観的かつ検証可能な収束で置き換えるために存在します。"],
            es: ["La mayoria de los sistemas de IA descentralizada todavia dependen de humanos para decidir que modelo es mejor. Eso significa que el cuello de botella de confianza nunca desaparecio; solo se tokenizo.", "RAXION existe para reemplazar esa capa subjetiva por convergencia objetiva y verificable entre sistemas de inferencia complementarios."],
          }
        ),
      }),
      section({
        heading: text("How it is built", {
          "pt-BR": "Como foi construida",
          zh: "如何构建",
          ja: "どのように構築されているか",
          es: "Como esta construida",
        }),
        paragraphs: lines(
          [
          ...
          {
            "pt-BR": ["O protocolo combina tres familias independentes de arquitetura, uma camada estatistica de coerencia, desafios estocasticos e sistemas criptograficos de prova. O site agora espelha essa estrutura com um sistema modular de conteudo.", "No whitepaper, essa arquitetura e formalizada como a Neural Sovereign Virtual Machine: um rollup soberano sobre Solana com scheduler cognitivo, Native Memory Accounts, Cross-Validation Neural e verificacao zk-ML nativa no proprio runtime."],
            zh: ["协议结合了三种独立架构、一层统计一致性、随机挑战以及密码学证明系统。网站现在也用模块化内容系统映射这一结构。", "在白皮书中，这套架构被正式定义为 Neural Sovereign Virtual Machine：一个基于 Solana 的主权 rollup，把认知调度器、Native Memory Accounts、Cross-Validation Neural 和原生 zk-ML 验证纳入运行时。"],
            ja: ["このプロトコルは 3 つの独立したアーキテクチャ、統計的一貫性レイヤー、確率的チャレンジ、暗号学的証明システムを組み合わせています。サイトも同様にモジュール化されたコンテンツ構成でこれを反映します。", "ホワイトペーパーでは、この構成は Neural Sovereign Virtual Machine として定式化されています。Solana ベースの主権ロールアップに、認知スケジューラ、Native Memory Accounts、Cross-Validation Neural、ネイティブ zk-ML 検証を組み込む設計です。"],
            es: ["El protocolo combina tres familias de arquitectura independientes, una capa estadistica de coherencia, desafios estocasticos y sistemas criptograficos de prueba. El sitio ahora refleja esa estructura con un sistema modular de contenido.", "En el whitepaper, esta arquitectura se formaliza como la Neural Sovereign Virtual Machine: un rollup soberano sobre Solana con scheduler cognitivo, Native Memory Accounts, Cross-Validation Neural y verificacion zk-ML nativa en el runtime."],
          }
        ),
      }),
      ...[
      ...
      ].map((spec) => sharedSection(spec as SharedSectionSpec)),
      section({
        heading: text("What comes next", {
        ...
        paragraphs: lines(
          ...
          {
            "pt-BR": ["Este blog sera o lugar para notas de protocolo, benchmarks, diagramas, testes e atualizacoes de arquitetura. Os anuncios cobrem marcos imediatos; os posts guardam o raciocinio mais profundo."],
            zh: ["这个博客将承载协议说明、基准测试、图表、测试和架构更新。公告覆盖即时里程碑；博客文章承载更深层的推理。"],
            ja: ["このブログは、プロトコルノート、ベンチマーク、図表、テスト、アーキテクチャ更新の置き場になります。アナウンスは直近のマイルストーン、ブログ記事はより深い思考を担います。"],
            es: ["Este blog sera el lugar para notas del protocolo, benchmarks, graficos, tests y actualizaciones de arquitectura. Los anuncios cubren hitos inmediatos; los posts guardan el razonamiento mas profundo."],
          }
        ),
      }),
    ],
...
export const announcementPosts: Post[] = sortPostsByDateDesc([announcementRebuild()]);

export const blogPosts: Post[] = sortPostsByDateDesc([
  ...englishBlogSpecs.map(englishBlog),
  whitepaperLaunchPost(),
  whyRaxionExistsPost(),
]);
